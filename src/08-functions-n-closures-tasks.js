...
function getPolynom(...args) {
  ...
  return (x) => {
    if (argsLength === 1) return args[0];
    if (argsLength === 2) return x + args[1];
    if (argsLength === 3) return args[0] * x ** args[0] + args[1] * x + args[2];
    return null;
  };
}
...
function logger(func, logFunc) {
  return (...args) => {
    const funcArgs = args.map((el) => JSON.stringify(el)).join(',');
