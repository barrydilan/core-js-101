...
function angleBetweenClockHands(date) {
  ...
  const hoursAngle = 30 * hours + date.getUTCMinutes() * 0.5;
  const difference = Math.abs(hoursAngle - minutesAngle);
  return (Math.abs(Math.min(difference, 360 - difference)) * Math.PI) / 180;
}
